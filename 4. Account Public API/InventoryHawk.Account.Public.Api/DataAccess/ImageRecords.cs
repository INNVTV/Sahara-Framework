...
{
    public static class ImageRecords
    {
        public static List<ImageRecordGroupModel> GetImageRecordsForObject(string accountNameKey, string imageFormatGroupTypeNameKey, string objectId, bool listingsOnly = false)
        {
            ...
            if (imageFormats == null)
            {
                ...
                imageFormats = DataAccess.ImageFormats.GetImageFormatsHelper(accountNameKey, imageFormatGroupTypeNameKey);
            ...
            if (localCacheEmpty)
            {
                ...
                HttpRuntime.Cache.Insert(localCacheKey, imageFormats, null, DateTime.Now.AddSeconds(120), TimeSpan.Zero);
            ...
            */
            ...
            if (!listingsOnly)
            {
                cloudTable = cloudTableClient.GetTableReference("acc" + account.AccountID.ToString().Replace("-", "").ToLower() + imageFormatGroupTypeNameKey + "imgs");
            }
            else
            {
                cloudTable = cloudTableClient.GetTableReference("acc" + account.AccountID.ToString().Replace("-", "").ToLower() + imageFormatGroupTypeNameKey + "lstimgs");
            }
                    ...
                    else
                    {
                        ...
                        foreach (ImageRecordTableEntity tableEntity in imageRecordEntities)
                        {
                            if (tableEntity.ImageKey == imageFormat.ImageFormatGroupNameKey + "-" + imageFormat.ImageFormatNameKey)
                            {
                                ...
                                imageRecordGroupModel.ImageRecords.Add(ImageRecordsTransforms.TableEntity_To_ImageRecord(cdnEndpoint, tableEntity, imageFormat, imageFormat.Gallery));
                        ...
                        if (!recordExists)
                        {
                            ...
                            imageRecordGroupModel.ImageRecords.Add(ImageRecordsTransforms.ImageFormat_To_ImageRecord(imageFormat));
        ...
        internal static class ImageRecordsTransforms
        {
