...
namespace Sahara.Core.Platform.Partitioning.Sql.Scripts.Initialization
{
    internal class Initialization
    {
        ...
        /// <returns></returns>
        public bool InitializePartition(string accountDatabasePartitionName)
        {
            ...
            foreach (string folder in scriptsOrder)
            {
                foreach (var sqlScript in resourceNames.Where(o => o.StartsWith(assemblyName + ".Sql.Scripts.Initialization." + folder)))
                {
                    ...
                    using (var reader = new StreamReader(stream))
                    {
                        ...
                        {
                            executeNonQueryStatement(s, Sahara.Core.Settings.Azure.Databases.DatabaseConnections.DatabasePartitionSqlConnection(accountDatabasePartitionName));
                        }
                    }
                }
            }
