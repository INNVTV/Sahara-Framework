/*-----------------------------------------------------------------------------------------
  ...
  Used to destroy an account schema that has been partitioned into this database

  ----------------------------------------------------------------------------------------*/

CREATE PROCEDURE DestroySchema
(
...
/*-----------------------------------------------------------------------------------------
  ...
  Description: Drops all objects in a schema, then the schema itself
 
  Limitations:
  
    1. Schema name MUST start with an alphanumeric character
    2. If a table has a PK with XML or a Spatial Index then it wont work
       (workaround: drop that table manually and re run it)
    3. If the schema is referred by a XML Schema collection then it wont work
