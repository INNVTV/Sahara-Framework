...
namespace Sahara.Core.Platform.Provisioning.Sql.Scripts.Initialization
{
    internal class Initialization
    {
        ...
        /// <returns></returns>
        public bool InitializeAccountProvision(string schemaName, string accountDatabasePartitionName)
        {
            ...
            {
                foreach (var sqlScript in resourceNames.Where(o => o.StartsWith(assemblyName + ".Sql.Scripts.Initialization." + folder)))
                {
                    ...
                    using (var reader = new StreamReader(stream))
                    {
                        ...
                        {
                            //REPLACE [schemaname] with schemaName/  accountID:
                            var script = s.Replace("#schema#", schemaName);

                            executeNonQueryStatement(script, Sahara.Core.Settings.Azure.Databases.DatabaseConnections.DatabasePartitionSqlConnection(accountDatabasePartitionName));
                        }
                    }
                }
            }
