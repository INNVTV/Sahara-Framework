...
{
    public class AccountUserProfilePhotoProcessor
    {
        /// <summary>
        ...
        /// <returns></returns>
        public DataAccessResponseType ProcessAccountUserProfilePhoto(string accountId, string storagePartition, byte[] imageByteArray)
        {
            ...
            var imageSpecs = Sahara.Core.Settings.Imaging.Images.AccountUserProfilePhoto;
            ...
            using (var sourceStream = new MemoryStream(imageByteArray))
            {
                ...
                foreach (var size in setSizes)
                {
                    ...
                    using (MemoryStream outStream = new MemoryStream())
                    {
                        ...
                        CloudBlobClient blobClient = Sahara.Core.Settings.Azure.Storage.GetStoragePartitionAccount(storagePartition).CreateCloudBlobClient();
            ...
            */
            ...
            var cdnEndpoint = Settings.Azure.Storage.GetStoragePartition(storagePartition).CDN;

            //Send the imageId in the response object
            response.isSuccess = true;
            response.SuccessCode = imageId;
            response.SuccessMessage = "https://" + cdnEndpoint + "/" + accountId + "/userphotos/" + imageId;
            return response;
        }
    }
}
