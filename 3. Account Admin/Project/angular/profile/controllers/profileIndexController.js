(function () {
    ...
    function profileIndexController(profileIndexServices, sharedServices) { //scaffoldIndexModels, sharedServices) {
        ...
        vm.otherAccountOwnerships = [] //<-- Other accounts this user is an owner of
        vm.otherAccountMemberships = [] //<-- Other accounts this user belongs to
       ...
       ==========================================*/
        ...
        $('#profilePhoto').hover(function () {
            $("#profilePhotoOverlay").fadeIn(120);
        }, function () {
            $("#profilePhotoOverlay").fadeOut(160);
        })
        ...
        vm.checkPhoto = function () {
            ...
            if (vm.currentUserProfile.Photo == "" || vm.currentUserProfile.Photo == null) {
                ...
                vm.currentUserProfile.Photo = vm.currentUserProfile.Photo;
            }
        }
       ...
       ==========================================*/
        ...
        function getOtherAccounts() {
            ...
            //Debug.trace("Getting other accounts...");

            profileIndexServices.getOtherAccounts()
            .success(function (data, status, headers, config) {

                //Debug.trace(vm.currentUserProfile);
                //Debug.trace(data);


                //Loop through and add to appropriate account arrays
                for (var i = 0; i < data.length; i++) {
                    if (data[i].AccountID != vm.currentUserProfile.AccountID) {
                        if (data[i].AccountOwner == true) {
                            vm.otherAccountOwnerships.push(data[i]);
                        } else {
                            vm.otherAccountMemberships.push(data[i]);
                        }
                    }

                }

                //Debug.trace("Got other accounts!");
       ...
       ==========================================*/
        ...
        function activate(){
            ...
            vm.applicationName = JSON.parse(CoreServiceSettings_ApplicationName);
            ...
            vm.checkPhoto(); //<-- create photo Url from ID
